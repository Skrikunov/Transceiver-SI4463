 ...
 */
 //#define SI446X_PACKET_SIZE 0x09
...
void __attribute__((weak, alias ("__empty_callback0"))) SI446X_CB_RXINVALIDPREAMBLE(void); // ДОПИСАЛ САМ (детектор ошибки преамбулы)
void __attribute__((weak, alias ("__empty_callback0"))) SI446X_CB_PREAMBLE(void); // ДОПИСАЛ САМ (детектор преамбулы)
void __attribute__((weak, alias ("__empty_callback0"))) SI446X_CB_RXINVALIDSYNC(void); // ДОПИСАЛ САМ (ошибка синхрослова)
...
void Si446x_read(void* buff, uint8_t len)
{
	SI446X_ATOMIC()
	{
		CHIPSELECT()
		{
			...
			for(uint8_t i=0;i<len;i++)						//читаем FIFO
				((uint8_t*)buff)[i] = spi_transfer(0xFF);
		}
	}
}
...
Serial.print("FTX="); //фиксированная длина
Serial.println(len);
}
else{
Serial.print("!FTX="); //нефиксированная длина
Serial.println(len);
}*/
	...
	SI446X_NO_INTERRUPT()
	{
		...
		SI446X_ATOMIC()
		{
			...
			{
				spi_transfer_nr(SI446X_CMD_WRITE_TX_FIFO);
#if !SI446X_FIXED_LENGTH										//если длина переменная
																//пусть массив из 5 эл-тов len=5
				spi_transfer_nr(len);							//пишем в FIFO[0] длину len=5
				for(uint8_t i=0;i<len;i++) 						//пишем в FIFO[1...5] элементы 5шт (0...4)
					spi_transfer_nr(((uint8_t*)packet)[i]);
#else															//если длина постоянная
				for(uint8_t i=0;i<SI446X_FIXED_LENGTH;i++) 		//пишем в FIFO[0...4] элементы 5шт
					spi_transfer_nr(((uint8_t*)packet)[i]);
#endif
			}
		}
		...
		setProperty(SI446X_PKT_FIELD_2_LENGTH_LOW, MAX_PACKET_LEN); //128 максимальный размер
#endif
	}
	return 1;
}
...
void Si446x_RX(uint8_t channel)
{
	SI446X_NO_INTERRUPT()
	{
		...
		uint8_t data[] = {
			SI446X_CMD_START_RX,//команда
			channel,			//канал
			0,					//немедленно начать принимать
			0,					//младший байт длины
			SI446X_FIXED_LENGTH,//старший байт длины
			SI446X_STATE_NOCHANGE, // RX Timeout Оставайтесь в состоянии RX, если происходит RXTIMEOUT.
			IDLE_STATE, // RX Valid, ready
...
*/
	...
	if(interrupts[4] & (1<<SI446X_PREAMBLE_DETECT_PEND)) //добавил детектор преамбулы
		SI446X_CB_PREAMBLE();

	if(interrupts[4] & (1<<SI446X_INVALID_PREAMBLE_PEND)) //добавил детектор преамбулы
		SI446X_CB_RXINVALIDPREAMBLE();
